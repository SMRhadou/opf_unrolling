...
def metrics(loss, u: torch.Tensor, eps: float, multiplier: torch.Tensor | None):
    ...
    violated_mask = u >= eps
    violated = u * violated_mask.float()

    # assert u.numel() > 0
    metrics = dict(
        rate=(violated_mask.sum() / violated_mask.numel()).nan_to_num(),        # Number of constraints violated
...
# def loss_equality(u: torch.Tensor, multiplier: torch.Tensor, augmented_weight=0.0, averaged=True):
...
def loss_equality(u: torch.Tensor, multiplier: torch.Tensor, augmented_weight=0.0, averaged=True):
    ...
    if augmented_weight > 0:
        loss = loss + augmented_weight * u.pow(2).mean(dim=1)
...
def loss_inequality(u: torch.Tensor, multiplier: torch.Tensor, augmented_weight=0.0):
    ...
    # u <= 0, therefore we have violation when u > 0, loss = max(0, u)^2
    if augmented_weight > 0:
        loss = loss + augmented_weight * u.relu().pow(2).mean(dim=1)
    return loss, loss.mean()
...
def equality(
    ...
    return metrics(loss, u.abs(), eps, multiplier), loss_tensor
...
def inequality(
    ...
    if lower_multiplier is None or upper_multiplier is None:
        return metrics(None, u_all, eps, None), None

    lower_tensor, loss_lower = loss_inequality(
        u_lower, lower_multiplier[:, mask_lower], augmented_weight=augmented_weight
    )
    upper_tensor, loss_upper = loss_inequality(
        u_upper, upper_multiplier[:, mask_upper], augmented_weight=augmented_weight
    )
    loss = loss_lower + loss_upper
    loss_tensor = torch.cat((lower_tensor.unsqueeze(1), upper_tensor.unsqueeze(1)), dim=1)
    ...
    return metrics(loss, u_all, eps, multiplier_all), loss_tensor
