...
from src.opf.hetero import HeteroGCN
...
class OPFDual(pl.LightningModule):
    ...
    def init_multipliers(self, n_bus, n_gen, n_branch, multiplier_table_length: int):
        """
        Initialize the inequality multipliers with ones and equality with zeros 
        """
        with torch.no_grad():
    ...
    def get_multipliers(self, idx: torch.Tensor) -> dict[str, torch.Tensor]:
        """
        only if we assign multipliers to each example in the dataset.
        ...
        multipliers = torch.zeros(idx.shape[0], self.n_multipliers, device=self.device)   # B x 420
        if self.common:
            multipliers = multipliers + self.multipliers_common.expand(idx.shape[0], -1)    # Use the common multipliers
        ...
        return multiplier_dict              # When self.common is False, we return zeros
    ...
    def add_args(parser: argparse.ArgumentParser):
        ...
        group.add_argument("--no_common", dest="common", action="store_false", default=True)
    ...
    def forward(
        ...
        if isinstance(data, HeteroData):
            ...
            load = data["bus"].x[:, :2].view(n_batch, powerflow_parameters.n_bus, 2)    # Load data
            bus = y_dict["bus"].view(n_batch, powerflow_parameters.n_bus, 4)[..., :2]   # predicted voltage B x n_bus x 2
            gen = y_dict["gen"].view(n_batch, powerflow_parameters.n_gen, 4)[..., :2]   # predicted generator's active and reactive power B x n_gen x 2
            branch = y_dict["branch"].view(n_batch, powerflow_parameters.n_branch, 4)   # predicted forward and backward B x n_branch x 4
    ...
    def _step_helper(
        ...
        constraints = self.constraints(variables, parameters, multipliers)      # Constraints * multipliers
        cost = self.cost(variables, parameters)                                 # OPF objective function
    ...
    def training_step(self, batch: PowerflowBatch):
        ...
        if self.current_epoch >= self.warmup:
            common_optimizer.step()                     # Dual Ascent Step
